...
find . -type f -iname "*.c" -path "$FOLDER_C_FILES" -maxdepth 1 -execdir gcc -c -fno-builtin -fno-stack-protector {} ";"

if [ "Darwin" == "$(uname -s)" ]; then
	used=$(find . -type f -iname "*.o" -path "$FOLDER_C_FILES" -maxdepth 1 -execdir nm -uA {} ";" | sed 's/\(.*\): \_/\1:/g' | sed 's/\.o:/.c:/')
else
	used=$(find . -type f -iname "*.o" -path "$FOLDER_C_FILES" -maxdepth 1 -execdir nm -uA {} ";" | sed 's/.\/\(.*\): *U /\1:/g' | sed 's/\.o:/.c:/')
fi
